...
function LittlestTokyoModel({ ...props }) {
  ...
  useEffect(() => {
    if (actions && 'Take 001' in actions) {
  ...
  useFrame((state, delta) => {
    ...
    if (action) action.time = THREE.MathUtils.damp(action.time, (action.getClip().duration / 2) * offset, 100, delta);
    ...
    <Canvas dpr={[1, 2]} shadows camera={{ position: [0, 0, 10], near: 0.1, far: 1000 }}>
      ...
      <Suspense fallback={null}>
        ...
        <ScrollControls pages={3}>
          <LittlestTokyoModel scale={0.02} position={[0, 2.5, 0]} />
        </ScrollControls>
      </Suspense>
    </Canvas>
  );
