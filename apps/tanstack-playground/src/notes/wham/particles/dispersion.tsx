...
export function LikeButtonWithDispersion() {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <style>{`
        @keyframes wham-disperse-fadeOut { to { opacity: 0 } }
        @keyframes wham-disperse-fromCenter { from { top: 50%; left: 50% } }
        .wham-disperse-particle {
          position: absolute;
          ...
          animation:
            wham-disperse-fadeOut var(--fade-duration) forwards,
            wham-disperse-fromCenter var(--disperse-duration) forwards;
          pointer-events: none;
        }
      `}</style>
      <button
      ...
      >
        ...
        {particles.map((particle) => (
          <span
            key={particle.id}
            className="wham-disperse-particle"
            style={
              {
                top: `${particle.top}%`,
                left: `${particle.left}%`,
      ...
      <p className="text-xs text-gray-500">
        粒子從中心 (50%, 50%) 飛到隨機落點 — disperse {DISPERSE_DURATION}ms,fade {FADE_DURATION}ms
      </p>
    </div>
  );
}
